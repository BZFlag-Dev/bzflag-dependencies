...
if "%CONF%" == "debug" (
	nmake -f Makefile.vc mode=dll VC=14 WITH_DEVEL="%outputroot%" WITH_ZLIB=dll WITH_CARES=dll ENABLE_IDN=no ENABLE_WINSSL=yes GEN_PDB=no DEBUG=yes MACHINE=%ARCH%
) else (
	nmake -f Makefile.vc mode=dll VC=14 WITH_DEVEL="%outputroot%" WITH_ZLIB=dll WITH_CARES=dll ENABLE_IDN=no ENABLE_WINSSL=yes GEN_PDB=no DEBUG=no MACHINE=%ARCH%
)

set CURL_RESULT=%ERRORLEVEL%

if %CURL_RESULT% == 0 (
    cd "%srcroot%\curl\builds\libcurl-vc14-%ARCH%-%CONF%-dll-cares-dll-zlib-dll-ipv6-sspi-winssl"
